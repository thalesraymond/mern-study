...
describe('GetSingleJobUseCase', () => {
    let getSingleJobUseCase: GetSingleJobUseCase;
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        getSingleJobUseCase = new GetSingleJobUseCase(mockJobRepository, mockUserRepository);
    });
    ...
    describe('execute', () => {
        it('should retrieve a single job by id successfully', async () => {
            ...
            const mockValidateOwnership = vi.spyOn(getSingleJobUseCase['ownershipUseCase'], 'execute').mockResolvedValue();


            const result = await getSingleJobUseCase.execute({ userId, jobId });
        ...
        it('should throw NotFoundError if job not found', async () => {
            ...
            await expect(getSingleJobUseCase.execute({ userId, jobId: '60d5ec49e0d3f4a3c8d3e8b5' })).rejects.toThrow(NotFoundError);
        });
        ...
        it('should throw UnauthorizedError when user tries to retrieve a job they do not own', async () => {
            ...
            vi.spyOn(getSingleJobUseCase['ownershipUseCase'], 'execute').mockRejectedValue(new UnauthorizedError("Not authorized to access this route"));


            await expect(getSingleJobUseCase.execute({ userId, jobId })).rejects.toThrow(UnauthorizedError);
        });

        it('should throw NotFoundError if user not found', async () => {
            (mockUserRepository.getById as Mock).mockResolvedValue(null);

            await expect(getSingleJobUseCase.execute({ userId: '60d5ec49e0d3f4a3c8d3e8b6', jobId })).rejects.toThrow(NotFoundError);
        });
    });
});
