...
describe('UserController', () => {
    ...
    beforeEach(() => {
        userController = new UserController(mockUserRepository, mockJobRepository);
        req = {
            body: {},
    ...
    describe('register', () => {
        it('should return a 201 status code and user data on successful registration', async () => {
            const userData: UserPayload = { name: 'Test', lastName: 'User', email: 'test@example.com', password: 'password', location: 'Test Location' };
            const userDTO: UserDTO = { id: VALID_MONGO_ID, name: 'Test', lastName: 'User', email: 'test@example.com', role: 'USER', location: 'Test Location' };
            req.body = userData;
